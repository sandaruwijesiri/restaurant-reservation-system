...
public class CustomerResource {
    ...
    public Response login(@Valid LoginDTO dto) {
        List<Customer> matchingCustomers = Customer.list("nic = ?1 AND password = ?2", dto.nic, dto.password);
        ...
        if(matchingCustomers.size()>0){
            ...
            response = Response.status(Response.Status.UNAUTHORIZED).entity("Invalid NIC or password").build();
        }
        return response;
    }
    ...
    @Path("/register")
    @Transactional
    public Response create(@Valid CustomerDTO dto, @Context UriInfo uriInfo) {
        if (Customer.findByIdOptional(dto.nic).isPresent()) {
            throw new WebApplicationException("NIC already exists", Status.CONFLICT);
        }
